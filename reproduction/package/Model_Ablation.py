...
## IOA loss ablation
class PRESENT_ablateIOAloss(nn.Module):
    def __init__(
        self, 
...
class PRESENT_ablateGAT(nn.Module):
    def __init__(
        self, 
        rna_dim: Optional[int] = None, 
        cas_dim: Optional[int] = None,
        ...
        # Encoder
        self.rna_encoder = BayesianMLPEncoder(rna_dim, d_lat, d_hid=d_hid) if self.is_rna else None
        self.cas_encoder = BayesianMLPEncoder(cas_dim, d_lat, d_hid=d_hid) if self.is_cas else None
        self.adt_encoder = BayesianMLPEncoder(adt_dim, d_lat_adt, d_hid=(2*adt_dim//3 + d_lat_adt// 3 , adt_dim//3 + 2*d_lat_adt//3)) if self.is_adt else None
    ...
    def forward_encoder(self, rna, cas, adt, edge_index):
        ...
        if self.is_rna: 
            ...
            out.append(rna)
        if self.is_cas: 
            cas = self.cas_encoder(cas)
            out.append(cas)
        if self.is_adt: 
            adt = self.adt_encoder(adt)
            out.append(adt)
        if self.num_modalities > 1:
            out = torch.cat(out, -1)
            out = self.fusion_layer(out)
...
## RNA ZINB ablation
class PRESENT_ablateZINBdecoder(nn.Module):
    def __init__(
        self, 
        rna_dim: Optional[int] = None, 
        cas_dim: Optional[int] = None,
        ...
        # Decoder
#         self.rna_decoder = ZINBDecoder(d_lat, (d_hid[1], d_hid[0]), rna_dim, zero_inflaten=rna_zero_inflaten, is_recons=self.is_recons, basic_module=dec_basic_module) if self.is_rna else None
        self.rna_decoder = MLP_Module(d_lat, (d_hid[1], d_hid[0]), rna_dim) if self.is_rna else None
        self.cas_decoder = ZIPDecoder(d_lat, (d_hid[1], d_hid[0]), cas_dim, zero_inflaten=cas_zero_inflaten, is_recons=self.is_recons, basic_module=dec_basic_module) if self.is_cas else None
        self.adt_decoder = MLP_Module(d_lat, (adt_dim//3 + 2*d_lat_adt//3, 2*adt_dim//3 + d_lat_adt// 3), adt_dim) if self.is_adt else None
    ...
    def forward(self, rna_norm, rna_counts, rna_libsize, 
        ...
        nll_loss = 0
        mse_loss = 0
        if self.is_rna and self.is_recons:
            mse_loss += F.mse_loss(rna, rna_norm)
        if self.is_cas: 
            nll_loss += NLL_loss(cas_counts, cas[0], cas[1], cas[2], scale_factor=cas_libsize, ridge_lambda=cas_ridge_lambda)
            if self.is_recons: mse_loss += F.mse_loss(cas[3], cas_norm)
        if self.is_adt:
    ...
    def inner_inference(self, edge_index, rna_norm=None, cas_norm=None, adt_norm=None, impute=True, device=torch.device("cpu")):
        ...
        if self.is_rna: 
            rna_lat = rna_lat.cpu().numpy()
            if impute: rna_imputed = rna.cpu().numpy()
        if self.is_cas: 
            cas_lat = cas_lat.cpu().numpy()
            if impute: cas_imputed = cas[2].cpu().numpy()
        if self.is_adt: 
...
## CAS ZIP ablation
class PRESENT_ablateZIPdecoder(nn.Module):
    def __init__(
        self, 
        rna_dim: Optional[int] = None, 
        cas_dim: Optional[int] = None,
        ...
        self.rna_decoder = ZINBDecoder(d_lat, (d_hid[1], d_hid[0]), rna_dim, zero_inflaten=rna_zero_inflaten, is_recons=self.is_recons, basic_module=dec_basic_module) if self.is_rna else None
#         self.cas_decoder = ZIPDecoder(d_lat, (d_hid[1], d_hid[0]), cas_dim, zero_inflaten=cas_zero_inflaten, is_recons=self.is_recons, basic_module=dec_basic_module) if self.is_cas else None
        self.cas_decoder = MLP_Module(d_lat, (d_hid[1], d_hid[0]), cas_dim) if self.is_cas else None
        self.adt_decoder = MLP_Module(d_lat, (adt_dim//3 + 2*d_lat_adt//3, 2*adt_dim//3 + d_lat_adt// 3), adt_dim) if self.is_adt else None
    ...
    def forward(self, rna_norm, rna_counts, rna_libsize, 
        ...
        if self.is_rna: 
            nll_loss += NLL_loss(rna_counts, rna[0], rna[1], rna[2], scale_factor=rna_libsize, ridge_lambda=rna_ridge_lambda)
            if self.is_recons: mse_loss += F.mse_loss(rna[3], rna_norm)
        if self.is_cas and self.is_recons: 
#             nll_loss += NLL_loss(cas_counts, cas[0], cas[1], cas[2], scale_factor=cas_libsize, ridge_lambda=cas_ridge_lambda)
            mse_loss += F.mse_loss(cas, cas_norm)
        if self.is_adt:
            mse_loss += F.mse_loss(adt, adt_norm)
    ...
    def inner_inference(self, edge_index, rna_norm=None, cas_norm=None, adt_norm=None, impute=True, device=torch.device("cpu")):
        ...
        if self.is_cas: 
            cas_lat = cas_lat.cpu().numpy()
            if impute: cas_imputed = cas.cpu().numpy()
        if self.is_adt: 
            adt_lat = adt_lat.cpu().numpy()
            if impute: adt_imputed = adt.cpu().numpy()
...
## Prob decoder ablation
class PRESENT_ablateProbDecoder(nn.Module):
    def __init__(
        self, 
        rna_dim: Optional[int] = None, 
        cas_dim: Optional[int] = None,
...
## ablate BNN
class PRESENT_ablateBNN(nn.Module):
    def __init__(
        self, 
        rna_dim: Optional[int] = None, 
        cas_dim: Optional[int] = None,
        ...
        # Encoder
        self.rna_encoder = GATEncoder(rna_dim, d_lat, d_hid=d_hid) if self.is_rna else None
        self.cas_encoder = GATEncoder(cas_dim, d_lat, d_hid=d_hid) if self.is_cas else None
        self.adt_encoder = GATEncoder(adt_dim, d_lat_adt, d_hid=(2*adt_dim//3 + d_lat_adt// 3 , adt_dim//3 + 2*d_lat_adt//3)) if self.is_adt else None
        ...
        for epoch in pbar:
            ...
            for cur in train_loader:
                ...
                ioa_loss, nll_loss, mse_loss = self.forward(rna_norm_batch, rna_counts_batch, rna_libsize_batch,
                                                            ...
                                                            rna_ridge_lambda=rna_ridge_lambda, cas_ridge_lambda=cas_ridge_lambda)
                if self.num_modalities > 1 and self.is_recons:
                    loss = alpha1*nll_loss + alpha3*mse_loss + alpha4*ioa_loss
                elif self.is_recons:
                    loss = alpha1*nll_loss + alpha3*mse_loss
                else:
                    loss = alpha1*nll_loss
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_loss_list.append(loss.item())
            ...
            if self.num_modalities > 1 and self.is_recons:
                ioa_loss = np.mean(IOA_loss_list)
                mse_loss = np.mean(MSE_loss_list)
                pbar.set_postfix({'NLL_loss': nll_loss, 'MSE_loss': mse_loss, 'IOA_loss': ioa_loss, 'ES counter':early_stopping.counter, "ES patience": early_stopping.patience})
            elif self.is_recons:
                mse_loss = np.mean(MSE_loss_list)
                pbar.set_postfix({'NLL_loss': nll_loss, 'MSE_loss': mse_loss, 'ES counter':early_stopping.counter, "ES patience": early_stopping.patience})
            else:
                pbar.set_postfix({'NLL_loss': nll_loss, 'ES counter':early_stopping.counter, "ES patience": early_stopping.patience})
            if early_stopping.early_stop:
                print("Early stop the training process")
                break
        x_lat, omics_lat, omics_imputed = self.inner_inference(undirected_geo.edge_index, rna_norm, cas_norm, adt_norm, impute=impute)
...
## ablate IBA loss
class PRESENT_BC_ablateIBAloss(nn.Module):
    ...
    def forward(self, rna_norm, rna_counts, rna_libsize, 
        ...
        iba_loss = None
        ibp_loss = IBP_loss(x_lat, target_lat, batch_indices, tau=self.tau) if target_lat is not None else None
    ...
    def inner_inference(self, edge_index, rna_norm=None, cas_norm=None, adt_norm=None, batch_indices=None, impute=True, device=torch.device("cpu"), batch_fusion=True):
        self.to(device)
        self.eval()
        ...
        for epoch in pbar:
            ...
            for cur in train_loader:
                ...
                x_lat, nll_loss, mse_loss, ioa_loss, iba_loss, ibp_loss = self.forward(rna_norm_batch, rna_counts_batch, rna_libsize_batch,
                                                                                       cas_norm_batch, cas_counts_batch, cas_libsize_batch,
                                                                                       adt_norm_batch, edge_index,
                                                                                       batch_indices, target_lat=joint_lat_batch,
                ...
                if self.num_modalities > 1 and self.is_recons:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + alpha3*mse_loss + alpha4*ioa_loss + beta2*ibp_loss + beta3*gen_loss
                elif self.is_recons:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + alpha3*mse_loss + beta2*ibp_loss + beta3*gen_loss
                else:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + beta2*ibp_loss + beta3*gen_loss
            ...
            early_stopping(train_loss)
            pbar.set_postfix({'IBP_loss':ibp_loss, 'DISC_loss':disc_loss, 'GEN_loss':gen_loss,
...
## ablate IBP loss
class PRESENT_BC_ablateIBPloss(nn.Module):
    def __init__(
        self, 
        rna_dim: Optional[int] = None, 
        cas_dim: Optional[int] = None,
    ...
    def forward(self, rna_norm, rna_counts, rna_libsize, 
        ...
        x_lat, rna_lat, cas_lat, adt_lat = self.forward_encoder(rna_norm, cas_norm, adt_norm, edge_index)
        if self.num_modalities == 1 and stage==2: x_lat = self.fusion_layer(x_lat)
        iba_loss = IBA_loss(x_lat, positive_indices, negative_indices, tau=self.tau) if positive_indices is not None and negative_indices is not None else None
        ibp_loss = None
    ...
    def model_train(self, spa_mat, rna_counts=None, cas_counts=None, adt_counts=None, batch_label=None, impute=True,
        ...
        for epoch in pbar:
            ...
            for cur in train_loader:
                ...
                if self.num_modalities > 1 and self.is_recons:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + alpha3*mse_loss + alpha4*ioa_loss + beta1*iba_loss + beta3*gen_loss
                elif self.is_recons:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + alpha3*mse_loss + beta1*iba_loss + beta3*gen_loss
                else:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + beta1*iba_loss + beta3*gen_loss
                gen_optimizer.zero_grad()
                loss.backward()
                gen_optimizer.step()
            ...
            gen_loss = np.mean(GEN_loss_list)
            early_stopping(train_loss)
            pbar.set_postfix({'IBA_loss':iba_loss, 'DISC_loss':disc_loss, 'GEN_loss':gen_loss,
                              'ES counter':early_stopping.counter, "ES patience": early_stopping.patience})
            if early_stopping.early_stop:
                print("Early stop the second-stage training process")
                break
...
## ablate ADV loss
class PRESENT_BC_ablateADVloss(nn.Module):
    def __init__(
        self, 
        rna_dim: Optional[int] = None, 
        cas_dim: Optional[int] = None,
    ...
    def forward(self, rna_norm, rna_counts, rna_libsize, 
        ...
        return nll_loss, mse_loss, ioa_loss, iba_loss, ibp_loss
    ...
    def model_train(self, spa_mat, rna_counts=None, cas_counts=None, adt_counts=None, batch_label=None, impute=True,
                    negative_samples=0.25, rna_ridge_lambda=0.5, cas_ridge_lambda=0.5, 
                    epochs=100, lr=1e-3, weight_decay=1e-4, batch_size=320, patience=20, delta=1e-4,
                    alpha1=1.0, alpha2=1.0, alpha3=1.0, alpha4=1.0, beta1=1.0, beta2=1.0, device=torch.device("cuda:0")):
        if self.is_rna:
            assert rna_counts is not None and rna_counts.shape[1] == self.rna_dim, "Invalid input rna_counts"
            rna_counts = scipy.sparse.csr_matrix(rna_counts) if not scipy.sparse.issparse(rna_counts) else rna_counts
        if self.is_cas:
        ...
        for epoch in pbar:
            ...
            for cur in train_loader:
                ...
                if self.is_adt:
                    ...
                    adt_norm_batch = None
                nll_loss, mse_loss, ioa_loss, iba_loss, ibp_loss = self.forward(rna_norm_batch, rna_counts_batch, rna_libsize_batch,
                                                                      cas_norm_batch, cas_counts_batch, cas_libsize_batch,
                                                                      adt_norm_batch, edge_index,
                                                                      batch_indices=batch_indices, rna_ridge_lambda=rna_ridge_lambda, cas_ridge_lambda=cas_ridge_lambda, stage=1)
                bnn_loss = self.bnn_loss()
        ...
        for epoch in pbar:
            ...
            for cur in train_loader:
                ...
                bnn_loss = self.bnn_loss()
                if self.num_modalities > 1 and self.is_recons:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + alpha3*mse_loss + alpha4*ioa_loss + beta1*iba_loss + beta2*ibp_loss
                elif self.is_recons:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + alpha3*mse_loss + beta1*iba_loss + beta2*ibp_loss
                else:
                    loss = alpha1*nll_loss + alpha2*bnn_loss + beta1*iba_loss + beta2*ibp_loss
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_loss_list.append(loss.item())
            ...
            early_stopping(train_loss)
            if self.num_modalities > 1 and self.is_recons:
                mse_loss = np.mean(BNN_loss_list)
                ioa_loss = np.mean(IOA_loss_list)
                pbar.set_postfix({'NLL_loss':nll_loss, 'MSE_loss':mse_loss, 'BNN_loss':bnn_loss, 'IOA_loss':ioa_loss, 'IBA_loss':iba_loss, 
                                  'IBP_loss':ibp_loss, 'ES counter':early_stopping.counter, "ES patience": early_stopping.patience})
            elif self.is_recons:
                mse_loss = np.mean(BNN_loss_list)
                pbar.set_postfix({'NLL_loss':nll_loss, 'MSE_loss':mse_loss, 'BNN_loss':bnn_loss, 'IBA_loss':iba_loss, 
                                  'IBP_loss':ibp_loss, 'ES counter':early_stopping.counter, "ES patience": early_stopping.patience})
            else:
                pbar.set_postfix({'NLL_loss':nll_loss, 'BNN_loss':bnn_loss, 'IBA_loss':iba_loss, 'IBP_loss':ibp_loss, 
                                  'ES counter':early_stopping.counter, "ES patience": early_stopping.patience})
            if early_stopping.early_stop:
                print("Early stop the second-stage training process")
                break
