...
def mclust_R(adata, n_cluster, modelNames='EEE', use_rep='embeddings', key_added="mclust", random_seed=666):
    """
    ...
    embedding = (adata.obsm[use_rep] - adata.obsm[use_rep].mean(axis=0)) / adata.obsm[use_rep].std(axis=0)
    res = rmclust(rpy2.robjects.numpy2ri.numpy2rpy(embedding), n_cluster, modelNames)
    if res is not None:
        ...
        print("mclust failed on this dataset and return NULL")
        adata.obs[key_added] = ["none"] * adata.shape[0]
        adata.obs[key_added] = adata.obs[key_added].astype('category')
...
def ref_feature_alignment(adata_ref, adata, omics=None):
    ...
    """
    if omics is None or omics.upper() in ["RNA"]:
